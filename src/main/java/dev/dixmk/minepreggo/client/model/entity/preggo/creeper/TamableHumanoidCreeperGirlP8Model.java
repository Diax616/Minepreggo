...
public class TamableHumanoidCreeperGirlP8Model extends AbstractTamableHumanoidPregnantCreeperGirlModel<TamableHumanoidCreeperGirlP8> {
	public TamableHumanoidCreeperGirlP8Model(ModelPart root) {
		super(root, new HierarchicalModel<TamableHumanoidCreeperGirlP8>() {		
			@Override
			...
			@Override
			public void setupAnim(TamableHumanoidCreeperGirlP8 creeperGirl, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
			    ...
			    if (creeperGirl.getItemBySlot(EquipmentSlot.CHEST).isEmpty()) {		    	
			    	if (creeperGirl.getPregnancyPain() ==  PregnancyPain.FETAL_MOVEMENT) {
				    	this.animate(creeperGirl.loopAnimationState, BellyAnimation.FETAL_MOVEMENT_P8, ageInTicks);		    
			    	}
			    	else {
