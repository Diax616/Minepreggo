...
public class TamableHumanoidCreeperGirlP7Model extends AbstractTamableHumanoidPregnantCreeperGirlModel<TamableHumanoidCreeperGirlP7> {
	public TamableHumanoidCreeperGirlP7Model(ModelPart root) {
		super(root, new HierarchicalModel<TamableHumanoidCreeperGirlP7>() {		
			@Override
			...
			@Override
			public void setupAnim(TamableHumanoidCreeperGirlP7 creeperGirl, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
			    ...
			    if (creeperGirl.getItemBySlot(EquipmentSlot.CHEST).isEmpty()) {		    	
			    	if (creeperGirl.getPregnancyPain() ==  PregnancyPain.FETAL_MOVEMENT) {
				    	this.animate(creeperGirl.loopAnimationState, BellyAnimation.FETAL_MOVEMENT_P7, ageInTicks);		    
			    	}
			    	else {
