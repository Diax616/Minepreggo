...
public class TamableZombieGirlP8Model extends AbstractTamablePregnantZombieGirlModel<TamableZombieGirlP8> {
	...
	public TamableZombieGirlP8Model(ModelPart root) {
		super(root, new HierarchicalModel<>() {		
			...
			@Override
			public void setupAnim(TamableZombieGirlP8 zombieGirl, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
			    ...
			    if (zombieGirl.getItemBySlot(EquipmentSlot.CHEST).isEmpty()) {		    	
			    	if (zombieGirl.getPregnancyPain() ==  PregnancyPain.FETAL_MOVEMENT) {
				    	this.animate(zombieGirl.bellyAnimationState, BellyAnimation.FETAL_MOVEMENT_P8, ageInTicks);		    
			    	}
			    	else {
				    	this.animate(zombieGirl.bellyAnimationState, BellyAnimation.HIGH_BELLY_INFLATION, ageInTicks);		    
			    	}
