...
public class TamableZombieGirlP6Model extends AbstractTamablePregnantZombieGirlModel<TamableZombieGirlP6> {
	...
	public TamableZombieGirlP6Model(ModelPart root) {
		super(root, new HierarchicalModel<>() {		
			...
			@Override
			public void setupAnim(TamableZombieGirlP6 zombieGirl, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
			    ...
			    if (zombieGirl.getItemBySlot(EquipmentSlot.CHEST).isEmpty()) {		    	
			    	if (zombieGirl.getPregnancyPain() ==  PregnancyPain.FETAL_MOVEMENT) {
				    	this.animate(zombieGirl.bellyAnimationState, BellyAnimation.FETAL_MOVEMENT_P6, ageInTicks);		    
			    	}
			    	else {
