...
public class MonsterCreeperGirlArmorLayer<E extends AbstractCreeperGirl, M extends AbstractMonsterCreeperGirlModel<E>> extends RenderLayer<E, M> {   
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight,  E entity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (armorItem instanceof DyeableLeatherItem dyeable) {
            ...
            this.renderHead(poseStack, bufferSource, packedLight, armorHead, r, g, b, this.getArmorResource(entity, itemstack, EquipmentSlot.HEAD, null));
            this.renderHead(poseStack, bufferSource, packedLight, armorHead, 1.0F, 1.0F, 1.0F, this.getArmorResource(entity, itemstack, EquipmentSlot.HEAD, "overlay"));
        } else {
            this.renderHead(poseStack, bufferSource, packedLight, armorHead, 1.0F, 1.0F, 1.0F, this.getArmorResource(entity, itemstack, EquipmentSlot.HEAD, null));
        }
    ...
    private void renderHead(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, ModelPart head, float r, float g, float b, ResourceLocation armorResource) {
    ...
    private void renderTrim(ArmorMaterial material, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, ArmorTrim trim, ModelPart head) {
    ...
    private void renderGlint(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, ModelPart head) {
        head.render(poseStack, bufferSource.getBuffer(RenderType.armorEntityGlint()), packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
    }

    @SuppressWarnings("removal")
	private ResourceLocation getArmorResource(E entity, ItemStack stack, EquipmentSlot slot, @Nullable String type) {
    	ArmorItem item = (ArmorItem) stack.getItem();
    	String forgeTexturePath = item.getArmorTexture(stack, entity, slot, type);
    	
    	if (forgeTexturePath != null) {
			if (type != null &&	type.equals("overlay")) {
				return ARMOR_LOCATION_CACHE.computeIfAbsent(item.getArmorTexture(stack, entity, slot, "overlay"), ResourceLocation::new);
			}
    		return ARMOR_LOCATION_CACHE.computeIfAbsent(forgeTexturePath, ResourceLocation::new);
    	}
